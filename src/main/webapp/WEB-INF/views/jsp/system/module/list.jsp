									...
									<tbody>
										<c:forEach var="module" items="${modules }">
											<tr>
												...
												<td>
													<c:if test="${module.enabled==true }">可用</c:if>
													<c:if test="${module.enabled==false }">禁用</c:if>
												</td>
												<td>${module.priority }</td>
												<td class="text-center"><p class="fa ${module.icon} "/></td>
												<td><abbr title="${module.description }">${module.description }</abbr></td>
												<td>
													<a href="${contextPath}/system/module/edit/${module.moduleId}" class="btn btn-primary btn-xs">编辑</a>
													<a onclick="if(!confirm('确定要删除吗?'))return false;" href="${contextPath}/system/module/del/${module.moduleId}" class="btn btn-danger btn-xs">删除</a>
												</td>
											</tr>
										</c:forEach>
									</tbody>
